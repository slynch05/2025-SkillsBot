...
public class Utilities {
  ...
  /**
   * Creates a SmartDashboard print for a pose
   * @param printName Name of the dashboard print
   * @param printPose pose to be printed
   * @param printDegrees true if printing a degree value, false if radian
   */
  public static void printPose(String printName, Pose2d printPose, boolean printDegrees){
    //TODO
    double[] printArray = {printPose.getX(), printPose.getY()};
    if(printDegrees) {
      printArray[2] = printPose.getRotation().getDegrees();
    } else {
      printArray[2] = printPose.getRotation().getRadians();
    }
    SmartDashboard.putNumberArray(printName, printArray);
  }
}